...
# basic cell of ODE-LSTM with integrand function and integrating process between two cells
...
class Model_3D(nn.Module):
    ...
    def forward(self, x, time_ratio, timespans = 10, mask = None, pre_points = 9):
        device = x.device
        batch_size = x.size(0)
        seq_len = timespans
        ...
        x = x.permute(0, 2, 1)
        time_ratio = time_ratio.permute(0, 2, 1)
        ...
        for t in range(seq_len):
            ...
            # 1 * batch_size
            ts = ratio1[:, 0]
            new_h1 = self.rnn_cell.forward(new_h, ts)
