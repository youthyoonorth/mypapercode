...
% relative delta t (1.6ms / 160ms = 0.01)
dt = 0.01;
...
for v = 5 : 5 : 30
    ...
    for i = 1 + file_count : file_number + file_count
        % read files
        file_name = ['..\dataset\ODE_dataset(R1_test)_v' num2str(v) '\dataset_v' num2str(v) '_' num2str(i) '.mat'];
        load(file_name);
        % record normalized beamforming gains
        % third loop for different samples
        for j = 1 : file_size
            % mmWave received signals of beam training
            ...
            for t_count = 1 : t_length
                ...
                if(mod(t_count, 100) == 1) % beam training
                    % MUSIC
...
% final results
beam_loss_sery_total = reshape(beam_loss_sery_total / file_number, 100, 10, 6);
