...
def eval(model, loader, total, m):
                ...
                for j in range(out_shape[1]):
                    ...
                    if i % (m + 1) != 0:
                        ...
                        BL[loss_count, d_count] = BL[loss_count, d_count] + (beam_power[i, j, train_index] / max(
                            beam_power[i, j, :])) ** 2
                        BL_sample[loss_count, d_count, (batch_num-1)*32 + j] = (beam_power[i, j, train_index] / max(
                            beam_power[i, j, :])) ** 2
    ...
    return acur, losses, BL, BL_sample
...
def main():
    ...
    for velocity in [5,20]:
        ...
        # path2 = 'D:/communiations/beam_tracking/data_final/' + 'ODE_dataset(R1_test)_v' + str(velocity)
        path2 = 'D:/communiations/beam_tracking/newdata1/' + 'ODE_dataset(v2)_v' + str(velocity) + '_test'
        # path2 = '../../dataset/' + 'ODE_dataset(R1_test)_v' + str(velocity)
        ...
        sample_eval = np.zeros((10, m, 2560, t))
        ...
        for tt in range(t):
            ...
            acur, losses, BL, BL_sample = eval(model, eval_loader, total, m)
            acur_eval.append(acur)
            loss_eval.append(losses)
            BL_eval[:, :, tt] = np.squeeze(BL)
            sample_eval[:, :, :, tt] = np.squeeze(BL_sample)
            ...
            mat_name = 'random_sample_' + info + '.mat'
            sio.savemat(mat_name, {'sample_eval': sample_eval})
