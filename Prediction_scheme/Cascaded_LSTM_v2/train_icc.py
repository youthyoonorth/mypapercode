...
def main():
    ...
    for velocity in [5, 10, 15, 20, 25, 30]:
        ...
        version_name = 'ICC_ir_3CNN_1LSTM_160epoch_m=4_v2'
        # a = 0.2v m/s^2
            ...
            for e in range(epoch):
                ...
                while not done:
                    ...
                    if count == True:
                        ...
                        out_tensor = model(channel_train, pre_points = 4)
                        loss = 0
                        # d_sery = torch.linspace(0.2 - 1e-8, 0.8 - 1e-8, steps=4)
                        # d_sery = d_sery.to(device)
                        d_sery = np.linspace(0.2 - 1e-8, 0.8 - 1e-8, num=4)
                        ...
                        for batch_count in range(batch_size):
                            for loss_count in range(10):
                                ...
                                for d_count in range(m):
                                    ...
                                    loss += criterion(torch.squeeze(out_tensor[min_location, loss_count, batch_count, :]),
                                                      labels[batch_count, loss_count * (m + 1) + d_count + 1])/batch_size
